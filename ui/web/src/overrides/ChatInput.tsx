/**
 * Web override for ChatInput.
 *
 * This is a FULL COPY of the desktop ChatInput component with web-specific
 * modifications for feature-flag gating.  When the desktop version changes,
 * this file MUST be updated to stay in sync.
 *
 * Desktop source: ui/desktop/src/components/ChatInput.tsx
 *
 * Modifications vs desktop:
 *   - All relative imports rewritten to use the @desktop/ alias.
 *   - Feature-flag guards added around file-attachment, recipe-creation,
 *     working-directory, and extension-selection UI elements.
 */
import { AppEvents } from '@desktop/constants/events';
import React, { useRef, useState, useEffect, useMemo, useCallback } from 'react';
import { Bug, ChefHat, ScrollText } from 'lucide-react';
import { Tooltip, TooltipContent, TooltipTrigger } from '@desktop/components/ui/Tooltip';
import { Button } from '@desktop/components/ui/button';
import type { View } from '@desktop/utils/navigationUtils';
import Stop from '@desktop/components/ui/Stop';
import { Attach, Send, Close, Microphone } from '@desktop/components/icons';
import { ChatState } from '@desktop/types/chatState';
import debounce from 'lodash/debounce';
import { LocalMessageStorage } from '@desktop/utils/localMessageStorage';
import { DirSwitcher } from '@desktop/components/bottom_menu/DirSwitcher';
import ModelsBottomBar from '@desktop/components/settings/models/bottom_bar/ModelsBottomBar';
import { BottomMenuModeSelection } from '@desktop/components/bottom_menu/BottomMenuModeSelection';
import { BottomMenuExtensionSelection } from '@desktop/components/bottom_menu/BottomMenuExtensionSelection';
import { AlertType, useAlerts } from '@desktop/components/alerts';
import { useConfig } from '@desktop/components/ConfigContext';
import { useModelAndProvider } from '@desktop/components/ModelAndProviderContext';
import { useAudioRecorder } from '@desktop/hooks/useAudioRecorder';
import { toastError } from '@desktop/toasts';
import MentionPopover, { DisplayItemWithMatch } from '@desktop/components/MentionPopover';
import { COST_TRACKING_ENABLED } from '@desktop/updates';
import { CostTracker } from '@desktop/components/bottom_menu/CostTracker';
import { DroppedFile, useFileDrop } from '@desktop/hooks/useFileDrop';
import { Recipe } from '@desktop/recipe';
import { MessageQueue, QueuedMessage } from '@desktop/components/MessageQueue';
import { detectInterruption } from '@desktop/utils/interruptionDetector';
import { DiagnosticsModal } from '@desktop/components/ui/Diagnostics';
import { getSession, Message } from '@desktop/api';
import CreateRecipeFromSessionModal from '@desktop/components/recipes/CreateRecipeFromSessionModal';
import CreateEditRecipeModal from '@desktop/components/recipes/CreateEditRecipeModal';
import { getInitialWorkingDir } from '@desktop/utils/workingDir';
import { getPredefinedModelsFromEnv } from '@desktop/components/settings/models/predefinedModelsUtils';
import {
  trackFileAttached,
  trackVoiceDictation,
...
} from '@desktop/utils/analytics';
import { getNavigationShortcutText } from '@desktop/utils/keyboardShortcuts';
import { UserInput, ImageData } from '@desktop/types/message';
import { compressImageDataUrl } from '@desktop/utils/conversionUtils';
import { fetchCanonicalModelInfo } from '@desktop/utils/canonical';
...
export default function ChatInput({
  ...
  return (
    <div
      ...
      data-drop-zone={flags.features.fileAttachment ? "true" : undefined}
      onDrop={handleLocalDrop}
      onDragOver={handleLocalDragOver}
    >
      {flags.features.fileAttachment && (
        <input
          ref={fileInputRef}
          type="file"
      ...
      <div className="flex flex-row items-center gap-1 p-2 relative">
        {/* NOTE: DirSwitcher imported via @desktop/ bypasses the override plugin, so we gate it here directly */}
        {flags.features.workingDirectory && (
          <DirSwitcher
            className="mr-0"
        ...
        {flags.features.workingDirectory && <div className="w-px h-4 bg-border-primary mx-2" />}
        {flags.features.fileAttachment && (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
        ...
        {flags.features.fileAttachment && <div className="w-px h-4 bg-border-primary mx-2" />}
        ...
        <div className="flex flex-row items-center">
          ...
          {/* NOTE: BottomMenuExtensionSelection imported via @desktop/ bypasses the override plugin, so we gate it here directly */}
          {flags.features.extensionSelection && <div className="w-px h-4 bg-border-primary mx-2" />}
          {flags.features.extensionSelection && <BottomMenuExtensionSelection sessionId={sessionId} />}
          {flags.features.recipeCreation && sessionId && messages.length > 0 && (
            <>
              <div className="w-px h-4 bg-border-primary mx-2" />
        ...
        {flags.features.recipeCreation && sessionId && showCreateRecipeModal && (
        ...
        {flags.features.recipeCreation && recipe && showEditRecipeModal && (
